...
Void BufferCursor::erase(Byte eraser) {
	...
	if (this->segmentIndex_ == 0) {
		...
		this->segmentIndex_ = this->segment_.mass() - 1;
		goto Update;
	} else if (!this->atSegmentEnd())
		this->segment_.split(this->segmentIndex_);
...
Nat BufferCursor::getColumn() const noexcept {
	...
	Nat column = 1;
	for (Nat index = this->segmentIndex_; segment[index] != '\n'; ++column) {
		if (!index) {
			if (!segment.prior())
				break;
...
Bool BufferCursor::atLineEnd() const noexcept {
	if (this->atSegmentEnd()) {
		...
		return true;
	} else return this->segment_[this->segmentIndex_ + 1] == '\n'; 
}
...
Void BufferCursor::moveToLineStart() noexcept {
	do {
		...
		if (!this->segment_.prior())
			break;
		this->segment_ = *this->segment_.prior();
		this->segmentIndex_ = this->segment_.mass();
	Continue:
		--this->segmentIndex_;
...
Void BufferCursor::moveToLineEnd() noexcept {
	...
	} while (this->segment_[this->segmentIndex_] != '\n');
	this->moveLeft();
}

Void BufferCursor::setColumns() noexcept {
	for (; this->position_.column != this->column_;
	     ++this->position_.column, ++this->segmentIndex_) {
		if (this->segment_[this->segmentIndex_] == '\n')
			break;
		if (this->segmentIndex_ != this->segment_.mass())
			continue;
		this->segment_ = *this->segment_.next();
		this->segmentIndex_ = 0;
	}
