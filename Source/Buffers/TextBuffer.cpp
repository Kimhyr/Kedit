...
TextBuffer::TextBuffer(const char* filePath, Flag flags)
	...
	unique_ptr<Segment>* curr = &this->_root;
	for (;;) {
		for (;; curr = &curr->get()->next()) {
			byte byte;
			try { byte = file.get(); }
			catch (const exception&) { goto Escape; }
			try {
				curr->get()->write(byte);
				if (byte == '\n')
					++this->_height;
		...
		curr->get()->next() = make_unique<Segment>(*curr->get());
	}
Escape:
	// I hate how `ifstream`'s destructor automatically closes the file.
	return;
}

void TextBuffer::print() noexcept {
	// TODO: Create an iterator to make this *clean*.
	for (Segment& curr = *this->_root.get(); curr.next().get(); curr = *curr.next())
		curr.print();
}
